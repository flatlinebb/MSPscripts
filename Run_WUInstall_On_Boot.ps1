##
# Put this in a shortut in "shell:StartUp" (C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Startup)
#    C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe -NoLogo -ExecutionPolicy Bypass -Command "C:\Support\Run_WUInstall_On_Boot.ps1"
#    Start in: C:\Support
##
# Or run from a separate batch file located in the same folder as the PS script:
#    @ECHO OFF
#    powershell.exe -NoLogo "Start-Process powershell -ArgumentList '-ExecutionPolicy Bypass -NoLogo -NonInteractive -NoProfile -File "%~dp0Run_WUInstall_On_Boot.ps1"' -Verb RunAs"
#    exit
##
...
if ( !$Show ) 
{
    PowerShell -NoLogo -NoExit -File $MyInvocation.MyCommand.Path 1
    return
}
